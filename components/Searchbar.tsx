...
const SearchBar = () => {
  ...
  return (
    <form className="flexCenter md:max-w-[287px] w-full relative">
      
      <Combobox value={search} onChange={setSearch}>
        <div className="relative min-w-full mt-1">
          <div className="relative w-full cursor-default overflow-hidden rounded-lg  text-left outline-none text-small">
            <Combobox.Input
              className="w-full border-none py-[14px]  pl-[15px] pr-10 text-sm leading-5 text-gray-900 outline-0 bg-slate-900"
              displayValue={(tag: any) => tag.name}
              onChange={(event) => setSearch(event.target.value)}
            />
            <Combobox.Button
              className={`${search !== "" ? "hidden" : "absolute"
                } inset-y-0 left-0 flex gap-[10px] transparent  items-center pl-[15px]`}
            >
              <Image
